...
func newTestResponder(content *Content, timeout time.Duration, bytesPerSecond int, responseCode int) *Responder {
	return &Responder{
		Config: &Config{
			Content:        content,
			Timeout:        timeout,
...
func TestConfigureContentReader(t *testing.T) {
	t.Run("EmptyContent", func(t *testing.T) {
		content := &Content{}
		responder := newTestResponder(content, time.Second*5, 1000, http.StatusOK)
...
func TestServeHTTP(t *testing.T) {
	...
	t.Run("Timeout", func(t *testing.T) {
		content := &Content{Protocol: "file", Path: "/tmp/test.txt"}
		responder := newTestResponder(content, time.Millisecond*50, 1000, http.StatusOK)
